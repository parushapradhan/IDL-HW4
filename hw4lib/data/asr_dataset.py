...
class ASRDataset(Dataset):
    def __init__(
        ...
        for i in tqdm(range(self.length)):
            ...
            if self.partition != "test-clean":
                ...
                transcript_path = os.path.join(self.text_dir, self.text_files[i])
                transcript_arr = np.load(transcript_path, allow_pickle=True)
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        batch_feats  = [torch.FloatTensor(sample[0]).transpose(0, 1) for sample in batch]
        ...
        if self.partition != "test-clean":
            ...
            batch_shifted      = [sample[1].clone().detach().to(torch.long) for sample in batch]# B x T
            batch_golden       = [sample[2].clone().detach().to(torch.long) for sample in batch]  # B x T
            ...
            transcript_lengths = torch.LongTensor([s.size(0) for s in batch_shifted]) # B  
