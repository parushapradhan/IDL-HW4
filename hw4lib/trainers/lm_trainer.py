...
class LMTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        self.criterion = nn.CrossEntropyLoss(ignore_index=tokenizer.pad_id, label_smoothing=label_smoothing)
    ...
    def _train_epoch(self, dataloader) -> Tuple[Dict[str, float], Dict[str, torch.Tensor]]:
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                batch_size, seq_len, num_classes = raw_preds.shape
                # Would you need to change the shape of the inputs to the criterion?
                loss_input = raw_preds.view(-1, num_classes)
                loss_target = targets_golden.view(-1)
                # Hint: See the documentation for CrossEntropyLoss
                raw_loss = self.criterion(loss_input, loss_target)
    ...
    def _validate_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.inference_mode():
                ...
                loss_target = targets_golden.view(-1)
                # Hint: See the documentation for CrossEntropyLoss
                loss = self.criterion(loss_input, loss_target)
