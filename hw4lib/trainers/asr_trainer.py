    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                B, T, C = seq_out.shape
                seq_flat = seq_out.reshape(-1, C)
                gold_flat= targets_golden.reshape(-1)
                ce_loss = self.ce_criterion(seq_flat, gold_flat)
                
                
                # TODO: Calculate CTC loss if needed
                if self.ctc_weight > 0:
                    log_probs     = ctc_inputs['log_probs']       # (src_len, B, C)
                    input_lengths = ctc_inputs['lengths']         # (B,)
                    label_lengths = transcript_lengths             
                    ctc_loss = self.ctc_criterion(
                        log_probs, targets_golden, input_lengths, label_lengths
                    )
