...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.source_embedding = SpeechEmbedding(
            input_dim, d_model, time_reduction, reduction_method,dropout
        ) # Speech embedding
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        # x_ctc = x_enc.permute(1, 0, 2) 
        # log_probs = self.ctc_head(x_ctc) 
        # ctc_inputs = {'log_probs': log_probs, 'lengths': source_lengths}
